...
function GridBackground({
        ...
        return {
          ...
          gridLine:
            direction === 'horizontal'
              ? Math.floor(Math.random() * (safeRows - 1)) + 1
              : Math.floor(Math.random() * (safeCols - 1)) + 1,
          delay: Math.random() * 2,
          duration: speed + Math.random() * 2,
        };
      }),
    [count, beamColors, speed, safeCols, safeRows]
  );
  ...
  return (
    ...
    >
      ...
      >
        {/* Grid Cells */}
        {Array.from({ length: safeCols * safeRows }).map((_, index) => (
          <div
            key={index}
      ...
      {animatedBeams.map((beam) => {
        ...
        const horizontalPosition = (beam.gridLine / safeRows) * 100;
        const verticalPosition = (beam.gridLine / safeCols) * 100;
