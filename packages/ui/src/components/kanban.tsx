  ...
  const findContainer = React.useCallback(
    (id: UniqueIdentifier) => {
      ...
      return columnIds.find((key) => {
        const columnItems = columns[key];
        return columnItems && columnItems.some((item) => getItemValue(item) === id);
      });
    },
    ...
    (event: DragEndEvent) => {
      ...
      if (onMove && !isColumn(active.id)) {
        ...
        if (activeContainer && overContainer) {
          ...
          if (!activeColumnItems || !overColumnItems) {
            return;
          }

          const activeIndex = activeColumnItems.findIndex(
...
function KanbanColumnContent({ value, className, children }: KanbanColumnContentProps) {
  ...
  const itemIds = React.useMemo(() => {
    const columnItems = columns[value];
    return columnItems ? columnItems.map(getItemId) : [];
