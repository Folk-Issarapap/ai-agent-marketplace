...
import { useDataGrid } from '@workspace/ui/components/data-grid';
...
function DataGridTableBodyRowSkeletonCell<TData>({
  ...
  // Show default skeleton if no custom skeleton is provided
  const skeletonContent = children || <Skeleton className="h-4 w-full" />;
...
function DataGridTableBodyRow<TData>({
  ...
  return (
    <tr
      ...
      onClick={(e) => {
        // Don't trigger row click if clicking on dropdown menu or its content
